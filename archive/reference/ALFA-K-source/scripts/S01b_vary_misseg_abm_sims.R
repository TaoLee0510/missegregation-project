...
ensure_packages(c("parallel","alfakR","stringr"))

setup_and_run_abm <- function(rep_id,pmis,outDir){
  library(alfakR)
  wavelength <- 0.8
...
outDir <- "data/raw/ABM_varyMisseg/"
...
param_grid <- expand.grid(rep_id = reps, pmis=pmis, KEEP.OUT.ATTRS = FALSE)
...
clusterExport(cl, varlist = c("setup_and_run_abm", "outDir","gen_randscape"))
clusterEvalQ(cl, library(alfakR))

# Run in parallel
parLapplyLB(cl, param_list, function(params) {
  setup_and_run_abm(params$rep_id, params$pmis, outDir)
})

...
Rcpp::List run_karyotype_abm(
    ...
    Rcpp::IntegerVector lower_copy_numbers  = Rcpp::IntegerVector(0),
    Rcpp::IntegerVector upper_copy_numbers  = Rcpp::IntegerVector(0),
    Rcpp::CharacterVector excluded_karyotypes = Rcpp::CharacterVector(0)
)
{
  ...
  // Optional chromosome-specific hard support for a full GRF simulation.
  // These bounds represent biologically non-viable daughter states, not a
  // missing-fitness LUT boundary. Rejections are recorded for diagnostics.
  const bool use_copy_bounds = lower_copy_numbers.size() > 0 || upper_copy_numbers.size() > 0;
  std::vector<int> lower_bounds;
  std::vector<int> upper_bounds;
  if (use_copy_bounds) {
    if (lower_copy_numbers.size() != n_chr_types_sim || upper_copy_numbers.size() != n_chr_types_sim) {
      Rcpp::stop("Copy-number bounds must each have one value for every chromosome type (%d).", n_chr_types_sim);
    }
    lower_bounds.assign(lower_copy_numbers.begin(), lower_copy_numbers.end());
    upper_bounds.assign(upper_copy_numbers.begin(), upper_copy_numbers.end());
    for (int k = 0; k < n_chr_types_sim; ++k) {
      if (lower_bounds[k] < 0 || upper_bounds[k] < lower_bounds[k]) {
        Rcpp::stop("Copy-number bounds must be non-negative with lower <= upper at every chromosome.");
      }
    }
  }
  const auto is_within_copy_bounds = [&lower_bounds, &upper_bounds, use_copy_bounds](const std::vector<int>& cn) {
    if (!use_copy_bounds) return true;
    for (size_t k = 0; k < cn.size(); ++k) {
      if (cn[k] < lower_bounds[k] || cn[k] > upper_bounds[k]) return false;
    }
    return true;
  };
  std::set<std::vector<int>> excluded_states;
  for (int i = 0; i < excluded_karyotypes.size(); ++i) {
    std::vector<int> cn = parse_karyotype_string_abm(Rcpp::as<std::string>(excluded_karyotypes[i]), n_chr_types_sim);
    if (cn.empty()) Rcpp::stop("Invalid excluded karyotype.");
    excluded_states.insert(cn);
  }
  const auto is_allowed_state = [&excluded_states](const std::vector<int>& cn) {
    return !excluded_states.count(cn);
  };
  ...
  long long boundary_rejections = 0;
  if ((record_on_interval || record_on_cull) && n_chr_types_sim > 0) {
    ...
    for(const auto& pair : population) {
      if ((use_grf || fitness_map.count(pair.first)) && is_within_copy_bounds(pair.first) && is_allowed_state(pair.first)) { 
        if (pair.second > 0) { 
  ...
  for (int step = 1; step <= n_steps; ++step) {
    ...
    for (const auto& parent_cn : current_karyotypes_vec) {
      ...
      for (long long div_idx = 0; div_idx < n_divs; ++div_idx) {
        ...
        if (!daughters.first.empty() && (use_grf || fitness_map.count(daughters.first))) {
          if (is_within_copy_bounds(daughters.first) && is_allowed_state(daughters.first)) {
        ...
        if (!daughters.second.empty() && (use_grf || fitness_map.count(daughters.second))) {
          if (is_within_copy_bounds(daughters.second) && is_allowed_state(daughters.second)) {
    ...
    for(const auto& pair_change : net_changes_this_step) { // Renamed pair
      if ((use_grf || fitness_map.count(pair_change.first)) && is_within_copy_bounds(pair_change.first) && is_allowed_state(pair_change.first)) {
    ...
    for (auto it = population.begin(); it != population.end(); ) {
      if (it->second <= 0 || !is_within_copy_bounds(it->first) || !is_allowed_state(it->first) || (!use_grf && !fitness_map.count(it->first))) {
  ...
  results_over_time.attr("boundary_rejections") = Rcpp::NumericVector::create(static_cast<double>(boundary_rejections));

...
test_that("ABM excludes configured karyotypes from the initial population", {
  diploid <- paste(rep.int(2L, 22L), collapse = ".")
  aneuploid <- paste(c(3L, rep.int(2L, 21L)), collapse = ".")
  out <- alfakR::run_karyotype_abm(
    initial_population_r = stats::setNames(as.list(c(10L, 10L)), c(diploid, aneuploid)),
    fitness_map_r = stats::setNames(list(), character(0)), p_missegregation = 0,
    dt = 0.1, n_steps = 1L, max_population_size = 100L,
    grf_centroids = matrix(0, nrow = 1L, ncol = 22L), grf_lambda = 1,
    lower_copy_numbers = rep.int(0L, 22L), upper_copy_numbers = rep.int(8L, 22L),
    excluded_karyotypes = diploid
  )
  expect_false(diploid %in% names(out[[1L]]))

...
predict_cached_krig <- function(object, x, dist_mat, Z = NULL, drop.Z = FALSE, just.fixed = FALSE) {
  ...
  null_call <- c(object$null.args, list(x = x_scaled))
  if ("Z" %in% names(formals(null_fun))) null_call$Z <- Z
  if ("drop.Z" %in% names(formals(null_fun))) null_call$drop.Z <- drop.Z
  Tmatrix <- do.call(null_fun, null_call)
